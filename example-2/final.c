...
int main() {
    ...
    for (int i = 0; i < proof_length; i++) {
        ...
        for (int k=0; k<proof[i].len; k++) {
            keccak_msg[k] = proof[i].data[k];           
        }
        keccak_update(proof[i].len);
        keccak_final();
        ...
        if (input.data[0] > 0xc0) {
            ...
            while (idx < input.len) {
                ...
                if (input.data[idx] <= 0x7f) {
                    decoded[num_strings].len = 1;
                    decoded[num_strings].data = &input.data[idx];
                    idx++;
                } else if (input.data[idx] <= 0xb7) {
                    unsigned int n = input.data[idx] - 0x80;
                    decoded[num_strings].len = n;
                    decoded[num_strings].data = &input.data[idx + 1];
                    idx += 1 + n;
                } else {
                    return 0;
                }
            ...
            if (num_strings == 17) {
                for (int k = 0; k < 32; k++) {
                    storage_hash[k] = decoded[nibbles[nibble_index]].data[k];
                }
                nibble_index++;
            } else {
