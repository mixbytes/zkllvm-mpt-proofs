...
void keccak256(const uint8_t* input, size_t inputLen, uint8_t output[32]) {
    ...
    // Initialize the Keccak context
    keccak_init(&ctx);

    // Update the Keccak context with the input data
    keccak_update(&ctx, input, inputLen);

    // Finalize the Keccak context and store the result in the 'output' buffer
