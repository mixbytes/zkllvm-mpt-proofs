...
Bytes* _decodeNibbles(Bytes *compact, uint8_t skipNibbles) {                                                                        
    ...
    length -= skipNibbles;                                                                                                             
    printf("Decoding nibbles: length: %d (skipped %d)\n", length, skipNibbles);
...
uint8_t* extractProofValue(uint8_t rootHash[32], Bytes* path, RLPItem** stack, int stackLength) {
    ...
    for (int i = 0; i < stackLength; i++) {
        ...
        //}
        if (numItems == 2) {
            printf("Inside numItems == 2, leaf or extension\n");
            Bytes *nodeKey;
            ...
            printf("Branch numitems == 17\n");
            printf("    mptKeyOffset: %d, len: %zu\n", mptKeyOffset, mptKey->length);
            if (mptKeyOffset != mptKey->length) {
...
int main() {
    ...
    //uint8_t pathFixed[] = {0x7f,0x0d,0x15,0xc7,0xfa,0xae,0x65,0x89,0x66,0x48,0xc8,0x27,0x3b,0x6d,0x7e,0x43,0xf5,0x8f,0xa8,0x42};
    //uint8_t pathFixed[] = {0x1};

    // key of slot 0 at 0xd19d4B5d358258f05D7B411E21A1460D11B0876F
    //uint8_t pathFixed[] = {0xea,0xa5,0x1a,0x4e,0x7f,0x1f,0x7a,0x70,0x9f,0xcc,0x8a,0x1d,0x27,0xd4,0x44,0x0c,0x3f,0x46,0x00,0xde,0x68,0xaa,0xbb,0xb3,0xf2,0xf9,0x80,0x24,0x2d,0x77,0xc3,0xc6};
    ...
    memcpy(path->data, keccakPath, sizeof(keccakPath));
...
uint8_t rlpproof8[]= {0xf8,0x70,0x9d,0x20,0x3d,0xb1,0x44,0xee,0x84,0xcf,0xbf,0xe3,0x90,0x1e,0xe8,0x36,0xb6,0xbf,0xe3,0x4d,0x25,0x86,0xb5,0xb6,0x74,0x3c,0xf2,0x16,0x80,0x28,0x29,0xb8,0xb8,0x50,0xf8,0x4e,0x1,0x8a,0x3,0x27,0x9e,0xe7,0xa5,0x54,0xa,0xc0,0x47,0xc8,0xa0,0x7b,0x21,0x70,0xeb,0x13,0x42,0x8,0x2d,0x77,0x52,0xa9,0x67,0xe,0xdb,0x1c,0x3f,0x18,0xbf,0x23,0xb8,0xaf,0x1d,0x23,0x13,0x6a,0x46,0x27,0xfe,0xa3,0x40,0xcf,0xf,0xa0,0x4d,0x9b,0xe6,0x48,0xc5,0xbf,0x39,0x97,0x36,0x70,0xd9,0xf8,0xb4,0x81,0xd5,0xd0,0xb9,0x71,0xe6,0xa2,0xdb,0x2d,0xec,0xcc,0x6b,0x98,0xcd,0xe2,0x1c,0x5d,0xd8,0x3e};
    ...
    uint8_t **proofs = malloc(9 * sizeof(uint8_t*));
    proofs[0] = rlpproof0;
    proofs[1] = rlpproof1;
    proofs[2] = rlpproof2;
    proofs[3] = rlpproof3;
    ...
    */
    ...
    extractProofValue(rootHash, path, rlpItems, 9);
