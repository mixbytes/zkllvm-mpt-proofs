...
uint64_t ctx_message[sha3_max_rate_in_qwords];
...
void keccak_update(uint16_t size) {
    ...
    if (idx) {
        ...
        for (uint16_t k = 0; k < (size < left ? size : left); k++) {
            ((char*)ctx_message)[idx + k] = keccak_msg[msgIndex + k];
        }
    ...
    while (size >= BLOCK_SIZE) {
        ...
        if (IS_ALIGNED_64(&keccak_msg[msgIndex])) {
            ...
            for (uint16_t k = 0; k < BLOCK_SIZE; k++) {
                ((char*)ctx_message)[k] = keccak_msg[msgIndex + k];
            }
            for (uint8_t i = 0; i < 17; i++) {
                ctx_hash[i] ^= le2me_64(((uint64_t*)ctx_message)[i]);
            }
...
void keccak_final() {
    ...
    for (uint16_t k = ctx_rest; k < BLOCK_SIZE; k++) {
        ((char*)ctx_message)[k] = 0;
    }

    ((char*)ctx_message)[ctx_rest] |= 0x01;
    ((char*)ctx_message)[BLOCK_SIZE - 1] |= 0x80;
    ...
    for (uint16_t i = 0; i < digest_length; i++) {
        keccak_hash[i] = (uint8_t)(ctx_hash[i / 8] >> (8 * (i % 8)));
    }
}

void ctx_init() {
    // Initialize ctx_hash to zero
    for (uint8_t i = 0; i < sha3_max_permutation_size; i++) {
        ctx_hash[i] = 0;
    }

    // Initialize ctx_message to zero
    for (uint8_t i = 0; i < sha3_max_rate_in_qwords; i++) {
        ctx_message[i] = 0;
    }

    // Initialize ctx_rest to zero
    ctx_rest = 0;
}
